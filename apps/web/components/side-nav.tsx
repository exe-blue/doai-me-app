...
const navItems = [
  ...
  { id: "signals", label: "전이" },
  { id: "work", label: "시스템" },
  { id: "principles", label: "철학" },
  { id: "colophon", label: "콜로폰" },
]
    ...
    <nav className="fixed left-0 top-0 z-50 h-screen w-16 md:w-20 hidden md:flex flex-col justify-center border-r border-border/30 bg-background/80 backdrop-blur-sm">
      <div className="flex flex-col gap-6 px-4">
        {navItems.map(({ id, label }) => (
          <button key={id} onClick={() => scrollToSection(id)} aria-label={`Scroll to ${label}`} className="group relative flex items-center gap-3">
            <span
              className={cn(
