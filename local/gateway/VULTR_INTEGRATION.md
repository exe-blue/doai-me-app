# Vultr Integration Guide

## 개요

기존 `gateway`에 Vultr `cloud-gateway` 연결을 추가하는 방법입니다.

## 아키텍처

```
┌─────────────────────────────────────────────────────────────────┐
│                        VULTR SERVER                             │
│                   cloud-gateway (Python)                        │
│                        WSS Hub                                  │
└────────────────────────┬────────────────────────────────────────┘
                         │ WSS Protocol v1.0
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│                     MINI PC (x15)                               │
│                   gateway (Node.js)                             │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  src/wss/VultrClient.js (NEW)                            │   │
│  │       → HELLO, HEARTBEAT, COMMAND 수신                   │   │
│  └──────────────────────────────────────────────────────────┘   │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  src/adb/           (EXISTING)                           │   │
│  │  src/adapters/laixi/ (EXISTING)                          │   │
│  └──────────────────────────────────────────────────────────┘   │
└────────────────────────────────────────────────────────────────-┘
                         │ USB/ADB
                         ▼
                   ┌───────────┐
                   │  S9 x20   │
                   └───────────┘
```

## 통합 방법

### 1. 환경 변수 설정 (.env)

```env
# Vultr 연결
VULTR_ENABLED=true
VULTR_URL=ws://158.247.210.152:8000/ws/node
NODE_ID=node_runner_001
NODE_SECRET=bXlfc3VwZXJfc2VjcmV0X2tleV8xMjM0NTY=
```

### 2. index.js 수정

```javascript
// index.js 상단에 추가
const { initVultrConnection, shutdownVultrConnection } = require('./vultr-integration');

// start() 함수 내, ADB 초기화 후에 추가
async function start() {
    // ... 기존 초기화 코드 ...
    
    // ADB 클라이언트 초기화 후
    await adbClient.initialize();
    
    // ===== Vultr 연결 추가 =====
    const vultrClient = await initVultrConnection({
        adbClient,
        laixiAdapter: null, // 또는 laixiAdapter 인스턴스
        logger,
        config
    });
    
    if (vultrClient) {
        logger.info('[Gateway] 🌐 Vultr 연결 활성화됨');
    }
    
    // ... 나머지 초기화 코드 ...
}

// shutdown() 함수에 추가
async function shutdown(signal) {
    // ... 기존 종료 코드 ...
    
    // Vultr 연결 종료
    shutdownVultrConnection();
    
    // ...
}
```

### 3. 커스텀 명령 핸들러 추가 (선택)

```javascript
const { getCommandExecutor } = require('./vultr-integration');

// 커스텀 명령 등록
const executor = getCommandExecutor();
if (executor) {
    executor.registerHandler('MY_CUSTOM_COMMAND', async (devices, params) => {
        // 커스텀 로직
        return {
            status: 'SUCCESS',
            device_results: devices.map(d => ({
                slot: d.slot,
                serial: d.serial,
                status: 'SUCCESS'
            }))
        };
    });
}
```

## 새로 추가된 파일

```
gateway/src/wss/
├── index.js           # 모듈 export
├── VultrClient.js     # Vultr WSS 클라이언트
└── CommandExecutor.js # COMMAND → ADB/Laixi 실행
```

## Protocol v1.0 메시지 흐름

```
NodeRunner                              Vultr Server
    │                                        │
    │ ① HELLO (node_id + signature)          │
    │ ─────────────────────────────────────▶ │
    │                                        │
    │ ② HELLO_ACK (session_id)               │
    │ ◀───────────────────────────────────── │
    │                                        │
    │ ③ HEARTBEAT (30초 주기)                 │
    │ ─────────────────────────────────────▶ │
    │                                        │
    │ ④ COMMAND (Pull-based Push)            │
    │ ◀───────────────────────────────────── │
    │                                        │
    │ ⑤ RESULT                               │
    │ ─────────────────────────────────────▶ │
```

## Laixi 없이 사용

Laixi가 없어도 직접 ADB로 동작합니다:

```javascript
await initVultrConnection({
    adbClient,
    laixiAdapter: null,  // ← null 또는 생략
    logger,
    config
});
```

CommandExecutor가 자동으로 ADB fallback 사용:
- Laixi 연결됨 → `laixiAdapter.tap()`, `laixiAdapter.swipe()`
- Laixi 없음 → `adbClient.shell('input tap ...')`

## 문제 해결

### 연결 안됨

```bash
# Vultr 서버 확인
curl http://158.247.210.152:8000/health

# 환경 변수 확인
echo $VULTR_URL
echo $NODE_ID
```

### 명령 실행 안됨

```bash
# ADB 연결 확인
adb devices

# 디바이스 상태 확인
curl http://localhost:3100/api/devices
```

