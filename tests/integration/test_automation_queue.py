...
class TestIdleTaskFlow:
    ...
    async def test_idle_task_flow(self, client):
        serial = f"IDLE_{datetime.now().timestamp()}"
        task = {'task_type': 'idle_watch', 'search_keyword': '테스트', 'priority': 5, 'status': 'pending'}
        try:
            r = client.table('automation_queue').insert(task).execute()
            task_id = r.data[0]['id']

            client.table('automation_queue').update({'status': 'completed'}).eq('id', task_id).execute()
...
class TestScheduledTask:
    ...
    async def test_scheduled_task_not_available_before_time(self, client):
        ...
        task = {'task_type': 'scheduled_watch', 'priority': 5, 'status': 'pending', 'scheduled_at': future}
        try:
            r = client.table('automation_queue').insert(task).execute()
            task_id = r.data[0]['id']

            now = datetime.now(timezone.utc).isoformat()
            query = client.table('automation_queue').select('*').eq('id', task_id)\
                .or_(f'scheduled_at.is.null,scheduled_at.lte.{now}').execute()
